...
static void pbx_retrieve_remote_capabilities(channelPtr c)
{
	pbx_assert(c != NULL);
...
#ifdef CS_SCCP_VIDEO
			if (	c->videomode != SCCP_VIDEO_MODE_OFF && c->rtp.video.instance) {
				if (c->rtp.video.receiveChannelState == SCCP_RTP_STATUS_INACTIVE || c->state != SCCP_CHANNELSTATE_HOLD) {
				...
				if ((c->rtp.video.receiveChannelState & SCCP_RTP_STATUS_ACTIVE) != 0) {
...
static PBX_CHANNEL_TYPE *sccp_astwrap_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *dest, int *cause)
{
	...
	*cause = AST_CAUSE_NOTDEFINED;
	...
	sccp_log(DEBUGCAT_CODEC) (VERBOSE_PREFIX_4 "remote caps: %s\n", ast_format_cap_get_names(ast_channel_nativeformats(requestor), &codec_buf));
	...
	*/
	...
	if (!channel->capabilities.video[0]) {
...
*/
		...
		//ast_format_cap *codecs = ast_rtp_instance_get_codecs(instance);
...
static void sccp_astwrap_getCodec(PBX_CHANNEL_TYPE * ast, struct ast_format_cap *result)
{
	...
	for (i = 0; i < ARRAY_LEN(channel->preferences.audio); i++) {
		...
		if (ast_format != ast_format_none) {
			//unsigned int framing = ast_format_get_default_ms(ast_format);
			unsigned int framing = ast_format_cap_get_format_framing(ast_channel_nativeformats(ast), ast_format);
...
static void add_payload_mapping_helper(struct ast_rtp_codecs *codecs, int payload_code, char *rtp_map_filter, const char *mime, int rate)
{
	sccp_log(DEBUGCAT_CODEC)(VERBOSE_PREFIX_3 "SCCP: update rtpmap: payload:%d, mime:%s, rate:%d\n", payload_code, mime, rate);
	ast_rtp_codecs_payloads_set_m_type(codecs, NULL, payload_code);
	if (ast_rtp_codecs_payloads_set_rtpmap_type_rate(codecs, NULL, payload_code, rtp_map_filter, (char *)mime, 0, rate)) {
		ast_rtp_codecs_payloads_unset(codecs, NULL, payload_code);
	}
}

static boolean_t sccp_astwrap_updateRtpMapping(constDevicePtr d,constChannelPtr c, sccp_rtp_t *rtp)
{
	boolean_t res = FALSE;
	PBX_RTP_TYPE *instance = rtp->instance;
	struct ast_rtp_codecs codecs = AST_RTP_CODECS_NULL_INIT;
	char *rtp_map_filter = NULL;
	skinny_codec_t * skinnyprefs = NULL;
	switch(rtp->type) {
		case SCCP_RTP_AUDIO:
			rtp_map_filter = "audio";
			skinnyprefs = (skinny_codec_t * )c->preferences.audio;
			break;
#if CS_SCCP_VIDEO
		case SCCP_RTP_VIDEO:
			rtp_map_filter = "video";
			skinnyprefs = (skinny_codec_t * )c->preferences.video;
			break;
#endif
		default:
	...
	for (skinny_codec_t i=SKINNY_CODEC_NONE; i<SKINNY_MAX_CAPABILITIES && skinnyprefs[i] != SKINNY_CODEC_NONE; i++) {
		...
		if (format != ast_format_none && format != ast_format_slin16) {
			int payload_code = ast_rtp_codecs_payload_code(ast_rtp_instance_get_codecs(instance), 1, format, 0);
			ast_rtp_codecs_payloads_set_m_type(ast_rtp_instance_get_codecs(instance), NULL, payload_code);
			const char *mime = ast_rtp_lookup_mime_subtype2(1, format, 0, 0);;
			int rate = ast_rtp_lookup_sample_rate2(1, format, 0);

			add_payload_mapping_helper(&codecs, payload_code, rtp_map_filter, mime, rate);
		}
	}
	// add exceptions
	if (rtp->type == SCCP_RTP_AUDIO) {
		if (SCCP_DTMFMODE_SKINNY == d->dtmfmode) {
			add_payload_mapping_helper(&codecs, 96, rtp_map_filter, "telephone-event", 0);
			add_payload_mapping_helper(&codecs, 105, rtp_map_filter, "cisco-telephone-event", 0);
			add_payload_mapping_helper(&codecs, 121, rtp_map_filter, "cisco-telephone-event", 0);
		}
		add_payload_mapping_helper(&codecs, 101, rtp_map_filter, "telephone-event", 0);				// RFC2833 DTMF
		add_payload_mapping_helper(&codecs, 25, rtp_map_filter, "L16", 16000);					// slin16 -> wideband
		//add_payload_mapping_helper(&codecs, 11, rtp_map_filter, "L16-256", 16000);				// slin16 -> wideband
		...
		/*
		sccp_astwrap_add_payload_mapping(&codecs, 96, rtp_map_filter, "H264", 16000);				// H264
		sccp_astwrap_add_payload_mapping(&codecs, 97, rtp_map_filter, "H264", 16000);				// H264
...
static boolean_t sccp_astwrap_createRtpInstance(constDevicePtr d, constChannelPtr c, sccp_rtp_t *rtp)
{
	...
	if (!sccp_astwrap_updateRtpMapping(d, c, rtp)) {
		pbx_channel_set_hangupcause(c->owner, AST_CAUSE_BEARERCAPABILITY_NOTAVAIL);
...
/*
	if (c->calltype != SKINNY_CALLTYPE_OUTBOUND) {
		...
		ast_format_cap_append_from_cap(caps, ast_channel_nativeformats(c->owner), AST_MEDIA_TYPE_UNKNOWN);
		ast_rtp_codecs_payload_formats(ast_rtp_instance_get_codecs(instance), peercaps, &peerNonCodecCapability);

		static int nonCodecCapability = AST_RTP_DTMF | AST_RTP_CISCO_DTMF;
		ast_format_cap_get_compatible(caps, peercaps, joint);
		...
		ast_debug(1, "Capabilities: us - %s, them - %s, combined - %s\n",
			ast_format_cap_get_names(caps, &global_buf),
			ast_format_cap_get_names(peercaps, &peer_buf),
			ast_format_cap_get_names(joint, &pvt_buf));		

		ast_debug(1, "Non-codec capabilities: us - %d, them - %d\n",nonCodecCapability, peerNonCodecCapability);

		sccp_astwrap_getSkinnyFormatMultiple(peercaps, ((sccp_channel_t *)c)->remoteCapabilities.audio, ARRAY_LEN(c->remoteCapabilities.audio));

		char buf[512];
		sccp_codec_multiple2str(buf, sizeof(buf) - 1, c->remoteCapabilities.audio, ARRAY_LEN(c->remoteCapabilities.audio));
		sccp_log(DEBUGCAT_CODEC) (VERBOSE_PREFIX_4 "updated remote caps: %s\n", buf);
		ao2_cleanup(caps);
		ao2_cleanup(joint);
		ao2_ref(peercaps, -1);
	}
...
static uint sccp_wrapper_get_dtmf_payload_code(constChannelPtr c)
{
	...
	if (SCCP_DTMFMODE_SKINNY != c->dtmfmode) {
		rtp_code = ast_rtp_codecs_payload_code(ast_rtp_instance_get_codecs(c->rtp.audio.instance), 0, NULL, AST_RTP_DTMF | AST_RTP_CISCO_DTMF);
	}
